...
class DentistsCO_Spider(scrapy.Spider):
    name = 'dentists_co'
    allowed_domains = ['hotfrog.com.co']
    start_urls = [
        'https://www.hotfrog.com.co/search/co/dentistas/%s' % page for page in range(1,4)
    ]

    # CSV settings
    custom_settings= {
        'FEED_URI': "hotfrog_CO_dentists" + datetime.datetime.now().strftime("_%Y_%m_%d_%H_%M") + ".csv",
           'FEED_FORMAT': 'csv',
           'FEED_EXPORT_FIELDS': [
               'company_phone',
               'company_name',
    ...
    def parse(self, response):
        ...
        for item in row_data:
            ...
            scraped_info = {
                ...
                # key:value
                'company_phone':             item[0].strip().replace('tel:+57','+57 '),
